...
| R-9a | **G8: server side CLOSED, client migration PENDING.** `POST /api/quiz/submit` returns an additive `gamification` block — `xp_awarded` plus the full `GET /api/gamification/me` snapshot taken right after the award, both built by `backend/services/gamification_service.py::me_snapshot` so the endpoint and the inline copy cannot disagree. Nothing is invented: `xp_awarded` is `null` when the XP write failed, and the whole block is `null` when the snapshot read failed. The client still does R-9's two-read subtraction in `useGamificationDelta.ts`; swapping it for `result.gamification` (typed `SubmitGamification` in `lib/quiz/types.ts`, currently optional) is the follow-up, and R-9's "omit rather than invent" rule carries over to both null cases unchanged. | The blank XP line R-9 tolerates was a race between two reads the server could answer in one. | Until the client migrates, the extra round trips stay and behaviour is exactly R-9's. |
...
- Abandon/discard endpoint (R-3) · feedback mode as a backend option (G1) · per-concept attempt history filter (G2) · finished-attempt
  review (G5) · tutor seed field (G6) · question `type` on the wire (G9) · flag persistence
...
- **G8 (XP/streak in the submit response) is no longer a seam on the backend** — submit returns the `gamification` block (R-9a).
  What remains is the client swap: `useGamificationDelta.ts` → `result.gamification`.
