...
> are historical; hashes change with every preamble revision.

`routes/learn.py` dispatches to the right mode via
`agent_for_mode(body.mode)`, with the same orchestrator-vs-legacy
