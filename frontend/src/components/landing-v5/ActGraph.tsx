...
const CAPTIONS = [
  {
    eyebrow: null,
    head: <>A hundred names you don&rsquo;t know <em style={{ color: '#8FD9A8' }}>yet.</em></>,
    body: "Every concept in your course becomes a node. Right now, they're strangers.",
  },
  {
    eyebrow: CAP_LABEL,
    head: <>You study. They <em style={{ color: '#8FD9A8' }}>connect.</em></>,
    body: 'Every quiz, note, and card you touch draws an edge. Structure appears.',
  },
  {
    eyebrow: CAP_LABEL,
    head: <>Mastery is the <em style={{ color: '#8FD9A8' }}>color.</em></>,
    body: null,
  },
];
...
const CAP_SCRIM: React.CSSProperties = {
  ...
  pointerEvents: 'none',
  background:
    'radial-gradient(ellipse 62% 58% at 38% 50%, rgba(4,18,12,0.82) 0%, rgba(4,18,12,0.66) 45%, rgba(4,18,12,0.28) 72%, rgba(4,18,12,0) 100%)',
  filter: 'blur(18px)',
};
...
const EYEBROW: React.CSSProperties = {
  ...
  letterSpacing: '0.34em', color: '#8FD9A8', textTransform: 'uppercase',
  textShadow: '0 1px 10px rgba(4,18,12,0.9)',
};
...
const CAP_H2: React.CSSProperties = {
  ...
  letterSpacing: '-0.02em', color: '#F6F8F4',
  textShadow: '0 2px 26px rgba(4,18,12,0.85)',
};

const CAP_P: React.CSSProperties = {
  position: 'relative', margin: '16px 0 0', color: '#B9D9C4',
  fontSize: 15.5, lineHeight: 1.7, textShadow: '0 1px 14px rgba(4,18,12,0.9)',
};

const MONO_TINY: React.CSSProperties = {
  fontFamily: "'JetBrains Mono',monospace", fontSize: 8.5,
  letterSpacing: '0.22em', color: '#7E9689',
};

/** Four blurred blobs that read as out-of-focus foliage at the frame edges. */
const FOLIAGE = [
  { style: { left: '-16%', top: '-12%', width: '46vw', height: '40vw', borderRadius: '48% 62% 55% 45%', background: 'radial-gradient(ellipse at 30% 30%, rgba(5,22,14,0.95), rgba(5,22,14,0) 68%)', filter: 'blur(34px)' } },
  { style: { right: '-18%', top: '-8%', width: '44vw', height: '38vw', borderRadius: '55% 45% 60% 50%', background: 'radial-gradient(ellipse at 70% 30%, rgba(4,18,12,0.92), rgba(4,18,12,0) 68%)', filter: 'blur(38px)' } },
  { style: { left: '-14%', bottom: '-14%', width: '44vw', height: '40vw', borderRadius: '60% 50% 45% 62%', background: 'radial-gradient(ellipse at 35% 60%, rgba(4,18,12,0.95), rgba(4,18,12,0) 68%)', filter: 'blur(36px)' } },
  { style: { right: '-15%', bottom: '-12%', width: '46vw', height: '42vw', borderRadius: '50% 60% 52% 48%', background: 'radial-gradient(ellipse at 65% 62%, rgba(5,22,14,0.92), rgba(5,22,14,0) 68%)', filter: 'blur(40px)' } },
];
    ...
    >
      <div
        style={{
          ...
          background:
            'linear-gradient(180deg, #081F14 0%, rgba(8,31,20,0.7) 6%, rgba(8,31,20,0) 16%), linear-gradient(0deg, #061710 0%, rgba(6,23,16,0.7) 5%, rgba(6,23,16,0) 14%), radial-gradient(ellipse 90% 80% at 50% 42%, #0E2C1D 0%, #081F14 62%, #061710 100%)',
        }}
      >
        ...
        >
                    ...
                    <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <span
                        style={{
                          ...
                          color: t === 'mastered' ? '#E6F2E8' : '#B9D9C4',
                        }}
                      >
          ...
          <div data-cap="3" style={{ position: 'absolute', left: 0, right: 0, bottom: '9vh', zIndex: 4, textAlign: 'center', opacity: 0, pointerEvents: 'none' }}>
            <span
              ...
              style={{
                ...
                background: 'radial-gradient(ellipse 54% 62% at 50% 50%, rgba(4,18,12,0.82) 0%, rgba(4,18,12,0.66) 45%, rgba(4,18,12,0.28) 72%, rgba(4,18,12,0) 100%)',
                filter: 'blur(18px)',
              }}
            />
            <h2
              style={{
                ...
                letterSpacing: '-0.025em', color: '#F6F8F4', textShadow: '0 3px 34px rgba(4,18,12,0.85)',
              }}
            >
              See what you <em style={{ color: '#8FD9A8' }}>know.</em>
            </h2>
            <p style={{ position: 'relative', margin: '14px 0 0', color: '#B9D9C4', fontSize: 16, textShadow: '0 1px 14px rgba(4,18,12,0.9)' }}>
              Your whole semester, one living map, updated as you learn.
            </p>
            <span style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 18, fontFamily: "'JetBrains Mono',monospace", fontSize: 10.5, letterSpacing: '0.22em', color: '#8FD9A8' }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#8FD9A8" strokeWidth="2" strokeLinecap="round">
          ...
          <div aria-hidden="true" style={{ position: 'absolute', right: 'max(4vw,24px)', top: '50%', transform: 'translateY(-50%)', zIndex: 4, display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[0, 1, 2, 3].map((i) => (
              <span key={i} data-tick={i} style={{ width: 5, height: 26, borderRadius: 99, background: 'rgba(230,242,232,0.2)', transition: 'background 300ms' }} />
            ))}
          </div>
        </div>
        ...
        {exploring && (
          <div style={{ position: 'absolute', inset: 0, zIndex: 6, pointerEvents: 'none', animation: 'panelFade 420ms ease 520ms both' }}>
            ...
            <div style={{ position: 'absolute', left: 'max(3vw,20px)', bottom: 26, display: 'flex', flexDirection: 'column', gap: 9, padding: '14px 16px', borderRadius: 14, background: 'rgba(6,23,16,0.66)', border: '1px solid rgba(143,217,168,0.18)', backdropFilter: 'blur(10px)', pointerEvents: 'auto' }}>
              <span style={{ ...MONO_TINY, letterSpacing: '0.24em' }}>
                {COURSE.term} · {Math.max(0, nodes.length - 1)} CONCEPTS MAPPED
              </span>
              {XTIER_ORDER.map((t) => (
                <span key={t} style={{ display: 'flex', alignItems: 'center', gap: 9, fontSize: 11.5, color: '#B9D9C4' }}>
                  <span style={{ width: 9, height: 9, borderRadius: 99, flex: '0 0 auto', background: XTIER[t] }} />
                  {XTIER_LABEL[t]}
                  <b style={{ marginLeft: 'auto', fontFamily: "'JetBrains Mono',monospace", fontSize: 10, fontWeight: 400, color: '#7E9689' }}>{counts[t]}</b>
                </span>
              ))}
            </div>

            <span style={{ position: 'absolute', left: '50%', bottom: 26, transform: 'translateX(-50%)', fontFamily: "'JetBrains Mono',monospace", fontSize: 9, letterSpacing: '0.22em', color: '#5F7A6C' }}>
            ...
            {node && (
              <div style={{ position: 'absolute', right: 'max(3vw,20px)', top: 78, bottom: 70, width: 'min(340px,32vw)', overflow: 'auto', padding: '22px 22px 20px', borderRadius: 18, background: 'rgba(6,23,16,0.82)', border: '1px solid rgba(143,217,168,0.22)', backdropFilter: 'blur(14px)', pointerEvents: 'auto', animation: 'panelFade 300ms ease both' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                  ...
                  <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 8, letterSpacing: '0.16em', color: '#5F7A6C' }}>
                    {node.root
                      ? 'COURSE'
                      : node.hub
                        ? `UNIT · ${COURSE.code}`
                  ...
                  <button
                    ...
                    type="button"
                    style={{ marginLeft: 'auto', width: 24, height: 24, borderRadius: 99, border: '1px solid rgba(143,217,168,0.22)', background: 'transparent', color: '#B9D9C4', cursor: 'pointer', fontSize: 13, lineHeight: 1 }}
                  >
                    ×
                  </button>
                </div>
                <h4 style={{ margin: '12px 0 0', fontFamily: "'Playfair Display',serif", fontSize: 26, fontWeight: 600, lineHeight: 1.15, color: '#F6F8F4' }}>{node.label}</h4>
                <p style={{ margin: '8px 0 0', fontSize: 13, lineHeight: 1.65, color: '#B9D9C4' }}>{node.blurb}</p>

                <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ flex: 1, height: 6, borderRadius: 99, background: 'rgba(230,242,232,0.12)', overflow: 'hidden' }}>
                  ...
                  <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, color: '#F6F8F4' }}>{Math.round(node.mastery * 100)}%</span>
                </div>
                <span style={{ display: 'block', marginTop: 5, ...MONO_TINY, letterSpacing: '0.16em', color: '#5F7A6C' }}>
                ...
                <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {neighbours.map((j) => (
                    <button
                      ...
                      style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 11px', borderRadius: 99, border: '1px solid rgba(143,217,168,0.2)', background: 'rgba(230,242,232,0.05)', color: '#E6F2E8', fontFamily: "'DM Sans',sans-serif", fontSize: 11.5, cursor: 'pointer', transition: 'all 200ms' }}
                    >
                ...
                <div style={{ marginTop: 9, display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {nodeUses(node, neighbours.length).map((u, i) => (
                    <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 3, paddingLeft: 11, borderLeft: '2px solid rgba(143,217,168,0.28)' }}>
                      <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 8.5, letterSpacing: '0.16em', color: '#8FD9A8' }}>{u.tag}</span>
                      <span style={{ fontSize: 12, lineHeight: 1.6, color: '#B9D9C4' }}>{u.text}</span>
                    </div>
                  ))}
                </div>

                <div style={{ marginTop: 18, display: 'flex', gap: 8 }}>
                  <button onClick={onQuiz} type="button" className="ld-expquiz" style={{ flex: 1, padding: '10px 12px', borderRadius: 9, border: 'none', background: '#0E9E5A', color: '#061710', fontFamily: "'DM Sans',sans-serif", fontSize: 12.5, fontWeight: 600, cursor: 'pointer', transition: 'background 200ms' }}>
                    Quiz me on this
                  </button>
                  <button onClick={onLearn} type="button" className="ld-explearn" style={{ flex: 1, padding: '10px 12px', borderRadius: 9, border: '1px solid rgba(143,217,168,0.3)', background: 'transparent', color: '#E6F2E8', fontFamily: "'DM Sans',sans-serif", fontSize: 12.5, cursor: 'pointer', transition: 'background 200ms' }}>
                    Ask the tutor
                  </button>
                </div>
                <span style={{ display: 'block', marginTop: 9, ...MONO_TINY, letterSpacing: '0.1em', color: '#5F7A6C' }}>
            ...
            <button
              ...
              style={{ position: 'absolute', right: 'max(3vw,20px)', top: 26, display: 'flex', alignItems: 'center', gap: 8, padding: '9px 16px', borderRadius: 99, border: '1px solid rgba(143,217,168,0.28)', background: 'rgba(6,23,16,0.7)', color: '#B9D9C4', fontFamily: "'JetBrains Mono',monospace", fontSize: 9.5, letterSpacing: '0.2em', cursor: 'pointer', pointerEvents: 'auto', transition: 'all 220ms' }}
            >
              ✕ EXIT EXPLORE
            </button>
          </div>
...
/** The band that lifts back out of the dark act into the light sections. */
  ...
  return (
    <div
      ...
      style={{
        ...
        background:
          'linear-gradient(180deg, #061710 0%, #0E2C1D 8%, #235139 20%, #46765C 34%, #74A288 48%, #A8C9B5 62%, #CFE2D5 76%, #E4EEE7 89%, #F0F4F2 100%)',
      }}
    />
  );
}
