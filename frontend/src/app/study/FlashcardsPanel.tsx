...
export default function FlashcardsPanel({ onStudyModeChange }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([getFlashcards(USER_ID), getCourses(USER_ID)])
      .then(([cardData, courseData]) => {
        setCards(cardData.flashcards ?? []);
        setCourses(courseData.courses.map((c: { course_name: string }) => c.course_name));
      })
      .catch(() => setCards([]))
      .finally(() => setLoading(false));
  }, [USER_ID, userReady]);

  const enterStudyMode = (active: boolean) => {
    setStudyMode(active);
    onStudyModeChange(active);
  };
  ...
  // ── Study mode — full-page (rendered by StudyClient at full height) ────────────
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 24px', gap: '24px' }}>
          <style>{`
            ...
            .flip-card-front, .flip-card-back { position: absolute; inset: 0; backface-visibility: hidden; -webkit-backface-visibility: hidden; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 40px; border-radius: 16px; border: 1px solid rgba(107,114,128,0.15); box-shadow: 0 8px 32px rgba(0,0,0,0.08); gap: 16px; cursor: pointer; user-select: none; }
            .flip-card-front { background: rgba(255,255,255,0.55); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border-color: rgba(255,255,255,0.68); }
            .flip-card-back { background: rgba(240,253,244,0.75); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); transform: rotateY(180deg); border-color: rgba(26,92,42,0.25); }
          `}</style>

          <div className="flip-card" onClick={() => setFlipped(f => !f)} style={{ width: '100%', maxWidth: '600px', minHeight: '280px', position: 'relative' }}>
            <div className={`flip-card-inner${flipped ? ' flipped' : ''}`} style={{ minHeight: '280px' }}>
              <div className="flip-card-front">
                <span style={{ fontSize: '11px', fontWeight: 600, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.09em' }}>Question</span>
                <p style={{ fontSize: '22px', fontWeight: 600, color: '#111827', textAlign: 'center', lineHeight: 1.5, margin: 0, letterSpacing: '-0.01em' }}>{currentCard.front}</p>
                <span style={{ fontSize: '12px', color: '#9ca3af', marginTop: '8px' }}>tap to reveal</span>
              </div>
              <div className="flip-card-back">
                <span style={{ fontSize: '11px', fontWeight: 600, color: '#1a5c2a', textTransform: 'uppercase', letterSpacing: '0.09em' }}>Answer</span>
                <p style={{ fontSize: '22px', fontWeight: 600, color: '#111827', textAlign: 'center', lineHeight: 1.5, margin: 0, letterSpacing: '-0.01em' }}>{currentCard.back}</p>
              </div>
            </div>
          </div>
          ...
          <button onClick={() => enterStudyMode(false)} style={{ fontSize: '13px', color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer' }}>
            Exit study mode
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ flex: 1, overflow: 'auto', display: 'flex' }}>
      {/* Left panel */}
      <div style={{
        width: '380px', flexShrink: 0, borderRight: '1px solid rgba(107,114,128,0.12)',
        padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: '20px', background: '#f9fafb',
      }}>
        <div>
          <p style={{ fontSize: '15px', fontWeight: 600, color: '#111827', margin: '0 0 12px' }}>Generate flashcards</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <label style={{ fontSize: '12px', color: '#6b7280' }}>Select a course to generate cards</label>
            {courses.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {courses.map(c => (
                  <button key={c} onClick={() => handleGenerate(c)} disabled={generating} style={{
                    padding: '10px 14px',
                    ...
                    opacity: generating && topic !== c ? 0.5 : 1, transition: 'all 0.15s',
                  }}>
                    <span>{c}</span>
                    {topic === c && generating
              ...
              <p style={{ fontSize: '12px', color: '#9ca3af', margin: 0 }}>No courses found. Add courses in the Dashboard first.</p>
            )}
          </div>
          {error && <p style={{ fontSize: '12px', color: '#dc2626', marginTop: '8px' }}>{error}</p>}
          {lastContextUsed && (
            <div style={{ marginTop: '10px', padding: '8px 12px', background: 'rgba(26,92,42,0.06)', border: '1px solid rgba(26,92,42,0.18)', borderRadius: '6px', fontSize: '12px', color: '#1a5c2a', lineHeight: 1.5 }}>
              ✦ Generated using{' '}
              {lastContextUsed.documents_found > 0 ? `${lastContextUsed.documents_found} library doc${lastContextUsed.documents_found > 1 ? 's' : ''}` : 'no library docs'}
              {lastContextUsed.weak_concepts_found > 0 ? ` · focused on ${lastContextUsed.weak_concepts_found} weak concept${lastContextUsed.weak_concepts_found > 1 ? 's' : ''}` : ''}
            </div>
          )}
        </div>
            ...
            return (
              <div key={card.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: '10px', fontWeight: 600, color: '#1a5c2a', background: 'rgba(26,92,42,0.08)', padding: '2px 8px', borderRadius: '10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{card.topic}</span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '4px' }}>
                  <span style={{ fontSize: '11px', color: '#9ca3af' }}>{card.times_reviewed > 0 ? `Reviewed ${card.times_reviewed}×` : 'Not reviewed'}</span>
                  <button onClick={() => handleDelete(card.id)} style={{ background: 'none', border: 'none', color: '#d1d5db', cursor: 'pointer', fontSize: '13px', padding: '2px 4px', borderRadius: '4px', lineHeight: 1 }}
