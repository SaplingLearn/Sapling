...
- backend/routes/internal_metrics.py — `GET /api/internal/metrics`, the server-to-server active-user counts (24h/7d/30d over `events`) the Canopy dashboard polls hourly. Own bearer token (`CANOPY_METRICS_TOKEN`; unset → 404), never `auth_guard`; counts come from the `canopy_active_users()` SQL function via `db.connection.rpc`, and anything it cannot vouch for is a 503, never zeros.
