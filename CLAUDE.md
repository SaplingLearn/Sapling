...
- All LLM calls are Pydantic AI agents in `backend/agents/` (model slots in `agents/_providers.py`); there is no other sanctioned LLM seam (ADR 0024). Exactly three raw `google.genai.Client` sites remain: `services/rag_service.py`'s embedding client (request-path, `model_mode()`-gated per #439), `scripts/_raw_gemini.py` (offline benchmark baseline — its docstring forbids importing it from application code), and `scripts/bench_quiz_prompt_budget.py` (offline `count_tokens` prompt measurement). The latter two are outside the request path and are never imported by application code.
