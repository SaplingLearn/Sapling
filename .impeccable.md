...
- **Atmospheric glass system — consistent across all surfaces** — this is a site-wide rule, not a landing page exception. Wherever an animated background (canvas nodes, atmospheric orbs, any living ambient layer) exists behind a UI element, that element uses atmospheric glass: `rgba(255,255,255,0.10)` fill, `blur(6px)` max, border `rgba(255,255,255,0.18)`. This applies to: navbar (when floating over canvas), dashboard sidebars and panels, floating cards, modals over animated backgrounds, and any surface the animation shows through. The effect must be felt, not seen — a whisper of frost that deepens the sense that the UI inhabits the atmosphere rather than sitting on top of it. On solid section backgrounds with no animation, surfaces are fully opaque white. The glass is earned by the animation beneath it, never applied as decoration.
