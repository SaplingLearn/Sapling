...
Run as evals (loads pydantic-evals; hits live Gemini):
    python -m tests.evals.syllabus_extraction
...
# Allow `python -m tests.evals.syllabus_extraction` from backend/.
...
async def _run(case_input: str) -> SyllabusAssignments:
    ...
    result = await syllabus_extraction_agent.run(case_input, deps=deps)
