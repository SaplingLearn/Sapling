...
-- Ordered migration applied by db/migrate.py (Postgres 15+ for NULLS NOT DISTINCT).
-- migrate.py runs each file in its own transaction, so the BEGIN/COMMIT below is
-- the unit of atomicity for the dedup + index build. Fresh DBs get these same two
-- UNIQUE indexes inline from migrations/0001_baseline_schema.sql; this migration
-- back-fills existing databases (dedup duplicates, then build the constraints).
--
