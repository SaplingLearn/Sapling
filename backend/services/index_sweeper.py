...
Each pass claims a batch through `claim_documents_for_indexing` — an atomic
`UPDATE ... RETURNING` over `SKIP LOCKED`, so two instances never drive the
same document — and re-drives each claimed row through the shared entry point,
...
def sweep_once() -> int:
    """Claim one batch and re-drive it. Returns how many were re-driven.
    ...
    RPC does not exist until the migration runs) is a warning, and one bad
    document never costs the rest of the batch.
    """
    ...
    try:
        ...
        logger.warning("[RAG] index sweep could not claim a batch", exc_info=True)
        return 0
